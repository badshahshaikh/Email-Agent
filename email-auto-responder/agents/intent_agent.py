...
class IntentAgent:
    def __init__(self, llama_agent):
        # SOLID: We inject the dependency instead of creating a new model
        self.llama_agent = llama_agent

    def process(self, text: str):
        """Uses the shared Llama instance to classify intents with high precision."""
        
        intent_prompt = """
        Analyze the banking email and identify specific intents. 
        Possible intents: "Account Balance", "Bank Statement", "Credit Card Transactions".
        Return ONLY a JSON list of objects: [{"intent": "name", "confidence": float}]
        """
        
        # We call the shared LLM through the agent
        result = self.llama_agent.process(text, custom_prompt=intent_prompt)
        
        audit_log = {
            "input_text": text,
            "raw_llm_output": result
        }
        audit_log("INTENT_AGENT", "INFO", audit_log)

        # Handle cases where LLM might return the whole object or just the list
